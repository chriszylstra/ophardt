   ...
   #     checkCam()
    ...
    if delay.value == "":
        ...
        if int(delay.value) < 2:
            app.warn(
                "Warning", "Min delay is 2 seconds due to scale communication.")
            delay.focus()
            return
        if int(dwell.value) < 2:
            app.warn(
                "Warning", "Min dwell is 2 seconds due to scale communication.")
            dwell.focus()
            return
        buttonStart.disable()
        buttonStop.enable()
        ...
        try:
            ...
            app.warn(
                "Warning", "USB not found. Data will not be saved.\n(Drive must be named exactly 'USB')\n\n" + str(e))
            usb_status = False
...
def cycle():
    ...
    global diff
    global usb_status
    dashboard.value = "Cycles Remaining: " + str(int(length.value)-remaining) + " (" + str(
        round((int(length.value)-remaining)*(int(delay.value)+int(dwell.value))/60, 1)) + " minutes)"
...
text_programName2 = Text(box3, text="_" + d1 +
                         ".csv        ", size=50, align="left")
programName.text_size = 50
text_fileLocation = Text(
    app, grid=[0, 8], text="Data file saved at: /media/pi/USB/", size=15, align="left")
blanktext3 = Text(app, text="", size=30, grid=[0, 8])

box5 = Box(app, grid=[0, 9], width="fill")
buttonStart = PushButton(box5, command=start_prog,
                         text="Start Pumpdown", width=20, height=2, align="left")
buttonStart.bg = "lightgreen"
buttonStart.text_size = 20
buttonStart.enable()
boxGap = Box(box5, width=30, height=3, align="left")
buttonPause = PushButton(box5, command=pause_prog,
                         text="Pause", width=20, height=2, align="left")
buttonPause.bg = "gold"
buttonPause.text_size = 20
buttonPause.disable()
boxGap2 = Box(box5, width=30, height=3, align="left")
buttonStop = PushButton(box5, command=stop_prog,
                        text="Stop", width=20, height=2, align="left")
buttonStop.bg = "tomato"
buttonStop.text_size = 20
buttonStop.disable()

box6 = Box(app, grid=[0, 11], width="fill")
buttonIn = PushButton(box6, command=deactivate,
                      text="Extend Arm", width=10, height=1, align="left")
buttonIn.bg = "royal blue"
buttonOut = PushButton(box6, command=activate,
                       text="Retract Arm", width=10, height=1, align="left")
...
dashboard = Text(box4, text="Cycles Remaining: " +
                 length.value, grid=[0, 0], size=20, align="left")
...
exiter = PushButton(app, command=exit_but, text="Exit",
                    width=20, height=2, grid=[0, 13], align="bottom")
exiter.text_size = 20
